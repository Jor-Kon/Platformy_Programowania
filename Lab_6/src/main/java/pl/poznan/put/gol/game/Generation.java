...
public class Generation {
	...
	public void evolve() {
		...
		for(Cell c : aliveCells){
			if(rules.inNextGeneration(isAlive(c), countAliveNeighbors(c))) nextGen.add(c);
		}
		for(Cell c : aliveCells.getNeighbors()){
			if(!aliveCells.contains(c) && rules.inNextGeneration(false, countAliveNeighbors(c))) nextGen.add(c);
		}
		aliveCells = nextGen;
	}
